"""EC063 — VAWT — F1a — Simulation & HTML Report"""
...
def generate_report():
    ...
    fig.add_trace(go.Scatter(x=v, y=r["power_coefficient"], name="Cp (VAWT)"), row=2, col=1)
    fig.add_trace(go.Scatter(x=v, y=np.full_like(v, 0.593), name="Betz", line=dict(dash="dash")), row=2, col=1)
    fig.add_trace(go.Scatter(x=v, y=np.full_like(v, 0.35), name="VAWT max", line=dict(dash="dot")), row=2, col=1)
