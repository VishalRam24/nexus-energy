"""EC062 — HAWT Onshore — F1a — Test Suite"""
...
def test_get_info(model):
    assert model.get_info()["ec_id"] == "EC062"
...
def test_rated_power_at_rated_speed(model):
    r = model.predict({"wind_speed": 14.0})
    assert 1900.0 < float(r["power_kw"]) <= 2000.0
...
def test_power_increases_with_wind(model):
    v = np.array([4.0, 6.0, 8.0, 10.0, 12.0])
...
def test_betz_limit(model):
    """Power coefficient should never exceed Betz limit (0.593)."""
