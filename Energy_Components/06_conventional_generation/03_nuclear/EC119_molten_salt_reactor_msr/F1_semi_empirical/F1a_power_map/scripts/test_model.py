"""EC119 -- Molten Salt Reactor (MSR) -- F1a Power-Map -- Test Suite"""
import sys, os, time
...
def run_tests():
    ...
    print("EC119 Molten Salt Reactor -- F1a Power Map")
    print("=" * 50)

    # Test 1: identity
    assert_true(info["ec_id"] == "EC119", "ec_id == EC119")
    ...
    # Test 3: rated output = 0.45 * 2250 = 1012.5 MW_e
    expected_el = 0.45 * 2250.0
    assert_true(abs(r["P_electric_mw"] - expected_el) < 0.1,
                f"P_electric at full load = {expected_el:.1f} MW_e (got {r['P_electric_mw']:.1f})")

    # Test 4: rated thermal = 2250 MW
    assert_true(abs(r["P_thermal_mw"] - 2250.0) < 0.1,
                f"P_thermal at full load = 2250 MW (got {r['P_thermal_mw']:.1f})")
    ...
    # Test 6: load factor clamping (min=0.3)
    r_low = model.predict({"load_factor": 0.1})
    assert_true(abs(r_low["load_factor_clamped"] - 0.3) < 1e-6, "LF clamped to min 0.3")
    r_high = model.predict({"load_factor": 1.5})
    assert_true(abs(r_high["load_factor_clamped"] - 1.0) < 1e-6, "LF clamped to max 1.0")

    # Test 7: eta constant across load
    for lf in [0.3, 0.6, 1.0]:
        r2 = model.predict({"load_factor": lf})
        assert_true(abs(r2["eta_thermal"] - 0.45) < 1e-6, f"eta=0.45 at LF={lf}")
