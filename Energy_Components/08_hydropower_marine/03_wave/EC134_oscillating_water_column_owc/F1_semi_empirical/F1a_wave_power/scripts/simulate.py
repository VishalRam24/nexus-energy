"""EC134 — OWC — F1a — Simulation Scenarios + HTML Report"""

import sys, json, numpy as np
from pathlib import Path
sys.path.insert(0, str(Path(__file__).parent))
from predict import ComponentModel


def run_simulation():
    model = ComponentModel()
    H_s_vals = np.linspace(0.5, 6.0, 50)
    T_e_vals  = np.array([7.0, 10.0, 13.0, 16.0])

    # Scenario 1: P vs H_s at various T_e
    curves = {}
    for T_e in T_e_vals:
        r = model.predict({"H_s": H_s_vals, "T_e": T_e})
        curves[T_e] = r["power_kw"]

    # Scenario 2: wave resource map (H_s x T_e grid)
    H_grid, T_grid = np.meshgrid(np.linspace(0.5, 6.0, 30), np.linspace(5.0, 20.0, 30))
    P_grid = model.predict({"H_s": H_grid, "T_e": T_grid})["power_kw"]

    # Scenario 3: sensitivity to CWR
    cwr_vals = np.linspace(0.1, 0.3, 20)
    P_cwr = [float(model.predict({"H_s": 2.5, "T_e": 10.0, "cwr": cwr})["power_kw"]) for cwr in cwr_vals]

    try:
        import plotly.graph_objects as go
        from plotly.subplots import make_subplots

        fig = make_subplots(rows=1, cols=3,
                            subplot_titles=["Power vs H_s (various T_e)",
                                            "Power Heatmap (H_s × T_e)",
                                            "Sensitivity to CWR (H_s=2.5m, T_e=10s)"])

        colors = ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728"]
        for i, T_e in enumerate(T_e_vals):
            fig.add_trace(go.Scatter(x=H_s_vals, y=curves[T_e],
                                     name=f"T_e={T_e}s", line=dict(color=colors[i])), row=1, col=1)

        fig.add_trace(go.Heatmap(z=P_grid,
                                  x=np.linspace(0.5, 6.0, 30),
                                  y=np.linspace(5.0, 20.0, 30),
                                  colorscale="Viridis", colorbar=dict(x=0.65, title="kW"),
                                  showscale=True), row=1, col=2)

        fig.add_trace(go.Scatter(x=cwr_vals, y=P_cwr,
                                  name="CWR sweep", line=dict(color="#9467bd")), row=1, col=3)

        fig.update_xaxes(title_text="H_s [m]", row=1, col=1)
        fig.update_yaxes(title_text="Power [kW]", row=1, col=1)
        fig.update_xaxes(title_text="H_s [m]", row=1, col=2)
        fig.update_yaxes(title_text="T_e [s]", row=1, col=2)
        fig.update_xaxes(title_text="CWR [-]", row=1, col=3)
        fig.update_yaxes(title_text="Power [kW]", row=1, col=3)

        fig.update_layout(title="EC134 OWC F1a — Simulation Report", height=500, width=1400)

        out = Path(__file__).parent.parent / "simulation_report.html"
        fig.write_html(str(out))
        print(f"Report saved: {out}")
    except ImportError:
        print("Plotly not available — skipping HTML report.")

    # Print summary table
    print("\n=== EC134 OWC — Power Output Summary ===")
    print(f"{'H_s (m)':<10} {'T_e=7s (kW)':<15} {'T_e=10s (kW)':<16} {'T_e=13s (kW)':<16}")
    for i, hs in enumerate(H_s_vals[::10]):
        row = [float(curves[T_e][i*10]) for T_e in [7.0, 10.0, 13.0]]
        print(f"{hs:<10.1f} {row[0]:<15.2f} {row[1]:<16.2f} {row[2]:<16.2f}")


if __name__ == "__main__":
    run_simulation()
