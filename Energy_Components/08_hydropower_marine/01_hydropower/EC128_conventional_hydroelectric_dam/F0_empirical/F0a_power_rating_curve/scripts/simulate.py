"""EC128 F0a — optional Plotly report (power & efficiency vs flow)."""
import numpy as np
...
def main():
    m = ComponentModel()
    Q = np.linspace(9, 33, 60)
    r = m.predict({"flow_rate_m3s": Q, "head_m": 100.0})
    try:
        ...
        fig.update_layout(title="EC128 F0a — Hydro power-rating curve", xaxis_title="Flow (m3/s)")
