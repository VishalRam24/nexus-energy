"""EC130 F0a — optional Plotly report (power & efficiency vs flow)."""
import numpy as np
...
def main():
    m = ComponentModel()
    Q = np.linspace(0.45, 1.65, 60)
    r = m.predict({"flow_rate_m3s": Q, "head_m": 40.0})
    try:
        ...
        fig.update_layout(title="EC130 F0a — Small-micro hydro power-rating curve", xaxis_title="Flow (m3/s)")
