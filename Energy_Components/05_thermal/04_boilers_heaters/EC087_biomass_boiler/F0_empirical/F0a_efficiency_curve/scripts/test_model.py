"""F0a tests for EC087 Biomass Boiler (Wood Pellet). Custom harness, no pytest. NumPy only."""
import os, sys, time
...
def main():
    ...
    assert_true(abs(r1["efficiency"] - 0.88) < 1e-6,
                "eta at PLR=1.0 equals eta_nom (0.88)")
    ...
    info = m.get_info()
    assert_true(info["component_id"] == "EC087" and info["fidelity"].startswith("F0a"),
