"""F0a tests for EC085 Natural Gas Boiler (Condensing). Custom harness, no pytest. NumPy only."""
import os, sys, time
...
def main():
    ...
    assert_true(abs(r1["efficiency"] - 0.95) < 1e-6,
                "eta at PLR=1.0 equals eta_nom (0.95)")
    rh = m.predict({"part_load_ratio": 0.5})
    assert_true(abs(rh["Q_out_kW"] - 0.5 * 50.0) < 1e-6, "Q_out = PLR*Q_rated")
    ...
    info = m.get_info()
    assert_true(info["component_id"] == "EC085" and info["fidelity"].startswith("F0a"),
