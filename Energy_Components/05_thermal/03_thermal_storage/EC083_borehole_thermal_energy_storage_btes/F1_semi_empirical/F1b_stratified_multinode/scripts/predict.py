"""EC083 — BTES — F1b Stratified Multi-Node — Standardized Predict Interface"""
import json, numpy as np
from pathlib import Path
from model import BoreholeTESF1b
...
class ComponentModel:
    ...
    def __init__(self, params: dict = None):
        ...
        self._model  = BoreholeTESF1b(defaults)

    def predict(self, inputs: dict) -> dict:
        q_ch  = float(inputs.get("q_charge_W",    0.0))
        q_dis = float(inputs.get("q_discharge_W", 0.0))
        T_amb = float(inputs.get("t_ambient",     self._model.T_ground_far))
        dur   = float(inputs.get("duration_s",    3600.0))
        dt    = float(inputs.get("dt",            300.0))
    ...
    def get_info(self) -> dict:
        return {
            ...
            "ec_id":       "EC083",
            "fidelity":    "F1b",
            "description": "Multi-node vertical stratification with ground losses and η_rt on charge",
            "inputs": {
                "q_charge_W":    {"unit": "W",    "range": [0, 500000]},
                "q_discharge_W": {"unit": "W",    "range": [0, 300000]},
                "t_ambient":     {"unit": "degC", "range": [-20, 30]},
                "duration_s":    {"unit": "s",    "range": [300, 2592000]},
                "T_initial":     {"unit": "degC", "default": "T_ground_undisturbed"},
                "dt":            {"unit": "s",    "default": 300},
            },
            "outputs": {
                "T_nodes":               {"unit": "degC"},
                "soc":                   {"unit": "-", "range": [0, 1]},
            ...
            "source": "Hellstrom (1991); Chapuis & Bernier (2009); Pahud & Matthey (2001)",
            "license": "BSD-3",
        }
...
if __name__ == "__main__":
    ...
    print(f"SI: {r['stratification_index']:.3f}")
