"""
EC044 -- Mono-Si PV -- F2a Diode Shading -- Test suite.
"""
import sys, os, time
import numpy as np
...
def test_uniform_stc():
    print("\n[Test 1] Uniform STC: P_mp near 280 W")
    m, _ = make()
    G = np.full(60, 1000.0)
    r = m.iv_curve(G, 25.0)
    assert_true(200 < r["P_mp"] < 350, f"P_mp={r['P_mp']:.1f} W in [200,350]")
...
def test_zero_irradiance():
    ...
    G = np.full(60, 0.0)
    r = m.iv_curve(G, 25.0)
    assert_true(r["P_mp"] < 1.0, f"P_mp={r['P_mp']:.2f} ~ 0")

def test_partial_shading_loss():
    print("\n[Test 3] Partial shading causes loss")
    m, _ = make()
    G_uniform = np.full(60, 1000.0)
    G_shaded = np.full(60, 1000.0)
    G_shaded[0:10] = 200.0  # shade 10 cells
...
def test_multiple_mpp():
    ...
    m, _ = make()
    G = np.full(60, 1000.0)
    G[0:20] = 100.0  # shade 1/3 of cells heavily
    r = m.iv_curve(G, 25.0, N_points=500)
    print(f"  Local maxima found: {r['num_local_maxima']}")
    # At least confirms the sweep ran correctly
...
def test_iv_shape():
    ...
    G = np.full(60, 1000.0)
    r = m.iv_curve(G, 25.0)
    # V should generally decrease (or at least not all increase)
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*60}\nEC044 PV F2a -- {p} passed, {f} failed\n{'='*60}")
