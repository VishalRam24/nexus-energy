"""EC051 — DSSC — F1a — Test Suite (no pytest)"""
import sys, time
import numpy as np
...
def test_physics_sanity():
    """Voc ~ 0.6-0.8V; FF > 0.4 at STC."""
    m = ComponentModel()
    r = m.predict({"G": 1000.0})
    assert_true(0.5 < float(r["Voc_V"]) < 0.9, f"Voc ~ 0.7V (got {float(r['Voc_V']):.3f})")
    assert_true(float(r["FF"]) > 0.40, f"FF > 0.40 (got {float(r['FF']):.3f})")
    assert_true(float(r["Pmp_W"]) > 0.0, "Pmp > 0 at STC")


def test_limits():
    """Pmp=0 at G=0; eta in (0,1)."""
    m = ComponentModel()
    r0 = m.predict({"G": 0.0})
    assert_true(float(r0["Pmp_W"]) == 0.0, "Pmp=0 at G=0")
    r1 = m.predict({"G": 1000.0})
    assert_true(0.0 < float(r1["eta"]) < 1.0, f"eta in (0,1) (got {float(r1['eta']):.3f})")


def test_monotonicity():
    """Pmp increases with G."""
    m = ComponentModel()
    gs = [100, 300, 500, 800, 1000]
...
def test_predict_interface():
    ...
    info = m.get_info()
    assert_true(info["ec_id"] == "EC051", "ec_id == EC051")


def test_benchmark():
    """10 predictions in < 5 s."""
    m = ComponentModel()
    N = 10
