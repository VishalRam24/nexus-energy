"""Standard ComponentModel interface for EC052 Bifacial PV Module (F0a)."""
import json
import os
import sys
...
class ComponentModel:
    ...
    def predict(self, inputs: dict) -> dict:
        """inputs: irradiance [W/m2], cell_temperature [degC], irradiance_rear [W/m2]."""
        g = float(inputs.get("irradiance", 1000.0))
        t = float(inputs.get("cell_temperature", 25.0))
        g_rear = float(inputs.get('irradiance_rear', 0.0))
        g = g + self.bifaciality * g_rear
