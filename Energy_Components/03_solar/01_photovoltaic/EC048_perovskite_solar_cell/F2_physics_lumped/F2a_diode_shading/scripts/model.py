"""
EC048 -- Perovskite Solar Cell -- F2a Diode + Partial Shading
...
Module: N_s cells in series (monolithic thin-film interconnect).
For a given module current I, each cell develops a voltage V_k.
If V_k < V_bypass, bypass diode activates.

Perovskite-specific physics vs silicon (EC044):
    - Higher bandgap (1.55 eV vs 1.12 eV) -> higher Voc per cell (~1.1 V)
    - Lower photocurrent (~20 mA/cm2 vs ~40 mA/cm2 for Si)
    - Higher ideality factor (n ~ 1.5-2.0, trap-assisted recombination)
    - Fewer cells per module (N_s ~ 24 for thin-film vs 60 for Si wafer)
    - Smaller temperature coefficient of Isc

References:
    Miyano et al. (2016), Acc. Chem. Res., 49, 303-310.
    Tress (2017), J. Phys. Chem. Lett., 8, 3106-3114.
    Bishop (1988), Solar Cells, 25, 73-89 (bypass diode model).
"""
...
class PerovskitePV_DiodeShading_F2a:
    """Perovskite PV module with cell-level single-diode + bypass diodes."""
    ...
    def _Vt(self, T_K):
        """Thermal voltage for single cell (includes ideality factor)."""
    ...
    def cell_voltage(self, I, G, T_K):
        ...
        # Search range: from reverse bias to Voc
        # Perovskite cells have higher Voc (~1.1 V) than Si (~0.65 V)
        V_lo = -5.0
        V_hi = 1.5
