"""EC046 — CdTe Thin-Film PV — F1a — Test Suite"""
...
def test_stc_power(model):
    """At STC, Pmp should be ~400-490W for First Solar Series 6 module."""
    r = model.predict({"irradiance": 1000.0, "cell_temperature": 25.0})
    assert 380.0 < float(r["p_mp"]) < 500.0, f"Pmp at STC = {float(r['p_mp']):.1f}W"
...
def test_efficiency_reasonable(model):
    """CdTe efficiency should be 14-19% at STC."""
    r = model.predict({"irradiance": 1000.0, "cell_temperature": 25.0})
    eff = float(r["efficiency"])
    assert 0.13 < eff < 0.20, f"Efficiency = {eff:.3f}, expected 0.14-0.19"


def test_temperature_derating(model):
    """CdTe should still produce significant power at 60C (>75% of STC)."""
    r25 = model.predict({"irradiance": 1000.0, "cell_temperature": 25.0})
    r60 = model.predict({"irradiance": 1000.0, "cell_temperature": 60.0})
    ratio = float(r60["p_mp"]) / float(r25["p_mp"])
    assert ratio > 0.75, f"Power retention at 60C = {ratio:.3f}, expected >0.75"
