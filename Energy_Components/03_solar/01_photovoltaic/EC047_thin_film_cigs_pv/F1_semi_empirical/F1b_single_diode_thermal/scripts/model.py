"""
EC047 — Thin-Film CIGS PV — F1b Single-Diode + Thermal Model

Extends F1a (De Soto 5-parameter single-diode) with:
  1. Faiman NOCT cell temperature model: T_cell = T_amb + G*(NOCT-20)/800
  2. Explicit power temperature coefficient gamma_pmp = -0.31 %/K for CIGS

CIGS context:
  - Bandgap ~1.15 eV (tunable); between CdTe (1.45 eV) and c-Si (1.12 eV)
  - Power tempco -0.31 %/K — better than poly-Si (-0.39 %/K), slightly worse
    than CdTe (-0.28 %/K)
  - De Soto model captures CIGS tempco accurately without need for the additional
    empirical correction required for CdTe, because CIGS bandgap and I_o temperature
    dependence are closer to the Si model assumptions

References:
    De Soto et al. (2006). Solar Energy 80(1), 78-88.
    Faiman (2008). Progress in Photovoltaics 16(4), 307-315.
    Solar Frontier SF170-S datasheet.
"""
...
class CIGSPvF1b:
    """CIGS thin-film PV — De Soto single-diode + Faiman NOCT thermal model."""
    ...
    def mpp_from_cell_temp(self, irradiance, cell_temp_c):
        """MPP given explicit cell temperature."""
        G = np.asarray(irradiance, dtype=float)
