"""EC047 — CIGS Thin-Film PV — F1a — Test Suite"""
...
def test_stc_power(model):
    """At STC, Pmp should be ~150-200W for SF170-S CIGS module."""
    r = model.predict({"irradiance": 1000.0, "cell_temperature": 25.0})
    assert 140.0 < float(r["p_mp"]) < 210.0, f"Pmp at STC = {float(r['p_mp']):.1f}W"
...
def test_efficiency_reasonable(model):
    """CIGS efficiency should be 12-17% at STC."""
    r = model.predict({"irradiance": 1000.0, "cell_temperature": 25.0})
    eff = float(r["efficiency"])
    assert 0.11 < eff < 0.18, f"Efficiency = {eff:.3f}, expected 0.12-0.17"
...
def test_temperature_derating(model):
    ...
    assert ratio > 0.75, f"Power retention at 60C = {ratio:.3f}"
