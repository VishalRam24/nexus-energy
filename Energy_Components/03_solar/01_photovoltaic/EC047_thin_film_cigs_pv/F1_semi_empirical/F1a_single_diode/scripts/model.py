"""
EC047 — Thin-Film CIGS PV — F1a Single-Diode Model

Pure-NumPy De Soto 5-parameter single-diode equation:

    I = I_L - I_o * [exp((V + I*R_s) / a) - 1] - (V + I*R_s) / R_sh

CIGS specifics:
    - Bandgap ~1.15 eV (close to single-junction optimum for AM1.5).
    - Direct-bandgap material -> high absorption coefficient, thin films suffice.
    - Power tempco ~ -0.31 %/K, similar to CdTe.
    - Module efficiency ~ 14-16% commercial.
...
class CIGSPVF1a:
    """CIGS thin-film PV — De Soto 5-parameter single-diode model (pure NumPy)."""
    ...
    def __init__(self, params: dict):
        ...
        self.area = mod.get("area", {"value": 1.232})["value"]
