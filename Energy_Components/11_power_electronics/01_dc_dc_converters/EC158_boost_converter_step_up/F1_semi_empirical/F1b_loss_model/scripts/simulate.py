"""EC158 -- Boost Converter -- F1b -- Simulation & HTML Report"""
...
def generate_report():
    ...
    for f in f_sw_range:
        ...
        m = BoostConverterF1b(params)
        eta_val = m.efficiency(12.0, 48.0, 5.0)
        etas_fsw.append(float(eta_val))
    fig.add_trace(go.Scatter(
        x=f_sw_range / 1e3, y=np.array(etas_fsw) * 100,
        name="eta vs f_sw (I=5A)", line=dict(color="#FF6692", width=2),
    ), row=2, col=2)
    ...
    # Summary table
    print(f"\n--- Boost Converter F1b Summary (V_in=12V, V_out=48V) ---")
    print(f"{'I(A)':>6} {'D':>6} {'I_in(A)':>8} {'eta%':>7} {'P_mos(W)':>9} "
          f"{'P_dio(W)':>9} {'P_sw(W)':>8} {'P_L(W)':>7} {'P_tot(W)':>9}")
    for i in [0.5, 1.0, 2.0, 3.0, 5.0, 8.0, 10.0]:
        rv = model.predict({"v_in": 12.0, "v_out_target": 48.0, "i_load": i})
        print(f"{i:>6.1f} {float(rv['duty_cycle']):>6.4f} "
              f"{float(rv['i_in']):>8.2f} "
