"""EC159 -- Buck-Boost Converter -- F1b Detailed Loss Model -- Standardized Predict Interface"""
...
class ComponentModel:
    def __init__(self, params_path=None):
        ...
        self._model = BuckBoostConverterF1b(self.params)

    def predict(self, inputs: dict) -> dict:
        """
        inputs:
            v_in          : float or array [V]
            v_out_target  : float or array [V]  (magnitude; output is inverted)
    ...
    def get_info(self) -> dict:
        ...
        return {
            ...
            "description": (
                ...
                "MOSFET conduction (I_L^2*D*R_ds_on(T)), "
                "diode conduction (I_out*V_f), "
                "switching (0.5*V_in*I_L*(t_on+t_off)*f_sw), "
                "inductor DCR (I_L^2*R_L), "
                "temp-dependent Rds_on, thermal balance T_j=T_a+P*R_theta"
            ),
            "inputs": {
            ...
            "params": {
                ...
                "R_ds_on_ref": f"{u['R_ds_on']['value']*1000:.0f} mohm at {u['T_ref']['value']} degC",
                "alpha_rds": f"{u['alpha_rds']['value']*1000:.1f} /degC (x1e-3)",
                "V_f": f"{u['V_f']['value']} V",
