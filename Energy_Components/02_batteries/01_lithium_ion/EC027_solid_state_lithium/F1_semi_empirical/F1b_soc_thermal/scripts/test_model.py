"""
EC027 -- Solid-State Lithium Battery -- F1b SOC-Thermal -- Test Suite

Physics sanity checks. SSE has higher E_a than liquid electrolyte cells.
...
def test_resistance_at_reference_equals_R_ref(model):
    ...
    assert abs(float(r) - 0.045) < 1e-6


def test_sse_high_e_a_strong_temperature_dependence(model):
    """SSE E_a=35 kJ/mol -> R(0C)/R(25C) should be >3 (stronger than liquid electrolyte)."""
    r_cold = float(model.predict({"soc": 0.5, "current": 0, "temperature": 273.15})["internal_resistance"])
    r_ref = float(model.predict({"soc": 0.5, "current": 0, "temperature": 298.15})["internal_resistance"])
    ratio = r_cold / r_ref
    # RATIONALE: SSE (sulfide) E_a~35 kJ/mol (Janek & Zeier 2016).
    # At 0C: ratio = exp(35000/8.314*(1/273.15-1/298.15)) ~ exp(1.42) ~ 4.1.
    # Threshold > 2.5 ensures E_a reflects SSE physics, not liquid electrolyte.
    assert ratio > 2.5, f"SSE R(0C)/R(25C)={ratio:.2f} should be >2.5 for E_a=35kJ/mol"


def test_voltage_lower_at_cold_high_current(model):
    v_cold = model.predict({"soc": 0.5, "current": 10.0, "temperature": 273.15})["terminal_voltage"]
    v_hot = model.predict({"soc": 0.5, "current": 10.0, "temperature": 333.15})["terminal_voltage"]
    assert float(v_cold) < float(v_hot), "Voltage at 0C must be lower than at 60C (high current)"
...
def test_array_inputs(model):
    ...
    temps = np.array([278.15, 298.15, 318.15])
...
def test_benchmark_1000_predictions(model):
    ...
    temps = np.random.uniform(273.15, 333.15, 1000)
