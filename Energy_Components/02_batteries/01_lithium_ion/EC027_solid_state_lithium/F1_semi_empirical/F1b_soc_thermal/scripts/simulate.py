"""
EC027 -- Solid-State Lithium Battery -- F1b SOC-Thermal -- Simulation & HTML Report Generator
"""
...
def generate_report():
    ...
    temperatures = [273.15, 283.15, 298.15, 313.15, 333.15]
    temp_labels = ["0C (273K)", "10C (283K)", "25C (298K)", "40C (313K)", "60C (333K)"]
    colors = ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd"]

    soc = np.linspace(0.05, 0.95, 200)
    discharge_current = 4.0  # A (1C for 4 Ah)

    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=[
            f"Terminal Voltage vs SOC at {discharge_current:.0f}A discharge (1C)",
            "Internal Resistance vs Temperature (SSE Arrhenius)",
    ...
    T_range = np.linspace(273.15, 333.15, 200)
    ...
    currents = np.linspace(0.0, 20.0, 200)
    for i, (T, label) in enumerate(zip(temperatures, temp_labels)):
