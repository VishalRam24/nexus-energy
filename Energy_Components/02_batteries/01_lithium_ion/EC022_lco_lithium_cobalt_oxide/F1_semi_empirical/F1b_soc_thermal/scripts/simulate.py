"""
EC022 -- LCO Battery -- F1b SOC-Thermal -- Simulation & HTML Report Generator

Generates interactive Plotly charts showing:
1. V vs SOC at different temperatures (273K, 283K, 298K, 308K, 318K)
...
def generate_report():
    ...
    temperatures = [273.15, 283.15, 298.15, 308.15, 318.15]
    temp_labels = ["0C (273K)", "10C (283K)", "25C (298K)", "35C (308K)", "45C (318K)"]
    colors = ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd"]

    soc = np.linspace(0.05, 0.95, 200)
    discharge_current = 2.6  # A (1C for 2.6 Ah cell)

    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=[
            f"Terminal Voltage vs SOC at {discharge_current:.1f}A discharge (1C)",
    ...
    T_range = np.linspace(273.15, 318.15, 200)
    ...
    currents = np.linspace(0.0, 10.0, 200)
    for i, (T, label) in enumerate(zip(temperatures, temp_labels)):
        q_vals = []
        for I in currents:
