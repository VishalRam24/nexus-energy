"""EC020 — NCA Battery — F1a SOC-Only — Simulation & HTML Report"""
...
def generate_report():
    ...
    fig.add_trace(go.Scatter(
        ...
        name="OCV", line=dict(color="#2ca02c", width=2.5)
    ), row=1, col=1)

    # Terminal voltage at various C-rates
    # Capacity = 3.35 Ah => 1C = 3.35 A
    C = 3.35
    c_rates = [0.5, 1.0, 2.0, 4.0]
    ...
    for crate, col in zip(c_rates, colors):
        ...
        fig.add_trace(go.Scatter(
            ...
            name=f"{crate}C ({I:.1f}A)", line=dict(color=col), legendgroup=f"C{crate}"
        ), row=1, col=2)
    ...
    fig.update_layout(
        title=f"{info['ec_id']} — {info['name']} — {info['fidelity']} SOC-Voltage Model<br>"
              f"<sup>Panasonic NCR18650B (NCA) | {info['source']}</sup>",
        height=850,
