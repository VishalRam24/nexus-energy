"""
EC018 -- LFP Battery -- F1b SOC-Thermal -- Test Suite
...
# --- LFP-specific: flat voltage plateau ---

def test_lfp_flat_plateau(model):
    """LFP OCV should be relatively flat in the 20-80% SOC range."""
    socs = np.linspace(0.3, 0.7, 50)
    result = model.predict({"soc": socs, "current": 0.0, "temperature": 298.15})
    ocv_range = float(np.max(result["ocv"]) - np.min(result["ocv"]))
    assert ocv_range < 0.6, f"LFP plateau should be flat, but OCV range = {ocv_range:.3f}V"
