"""
EC041 -- EDLC Supercapacitor -- F1b RC-Thermal -- Simulation & HTML Report Generator

EDLC operates from -40 to 65 degC, very wide temperature range.
"""
...
def generate_report():
    ...
    temperatures = [233.15, 258.15, 298.15, 318.15, 338.15]
    temp_labels = ["-40C (233K)", "-15C (258K)", "25C (298K)", "45C (318K)", "65C (338K)"]
    colors = ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd"]

    v_cap = np.linspace(0.1, 2.7, 200)
    discharge_current = 100.0  # A

    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=[
            f"Terminal Voltage vs V_cap at {discharge_current:.0f}A discharge",
            "ESR vs Temperature (Arrhenius)",
    ...
    T_range = np.linspace(233.15, 338.15, 200)
    result_T = model.predict({"v_cap": 1.35, "current": 0.0, "temperature": T_range})
    fig.add_trace(
    ...
    currents = np.linspace(0.0, 400.0, 200)
    for i, (T, label) in enumerate(zip(temperatures, temp_labels)):
        q_vals = [float(model.predict({"v_cap": 1.35, "current": I, "temperature": T})["heat_generation"])
                  for I in currents]
        fig.add_trace(
    ...
    result_cap = model.predict({"v_cap": 1.35, "current": 0.0, "temperature": T_range})
    fig.add_trace(
