"""
EC031 -- Sodium-Ion Battery -- F1b SOC-Thermal -- Test Suite
...
# --- Na-ion specific: lower voltage range ---

def test_naion_lower_voltage_than_nmc(model):
    """Na-ion nominal voltage (~3.1V) should be lower than NMC (~3.6V)."""
    result = model.predict({"soc": 0.5, "current": 0.0, "temperature": 298.15})
    ocv = float(result["ocv"])
    assert 2.5 <= ocv <= 3.5, f"Na-ion OCV at 50% SOC = {ocv:.3f}V, expected ~3.1V"
