"""EC035 — NaS Battery — F1a SOC-Only — Simulation & HTML Report Generator"""
...
def generate_report():
    ...
    c_rates = [0.0, 0.1, 0.2, 0.5, 1.0, 1.5]
    currents = [c * capacity_ah for c in c_rates]

    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=[
            "OCV vs SOC", "Terminal Voltage vs SOC (Discharge)",
            "Discharge Curve (V vs Delivered Ah)", "Power vs SOC (Discharge)",
        ],
        vertical_spacing=0.12, horizontal_spacing=0.1,
    )

    fig.add_trace(go.Scatter(x=soc, y=ocv_result["ocv"], name="OCV", line=dict(width=2)), row=1, col=1)

    colors = ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd", "#8c564b"]
    for i, (c, I) in enumerate(zip(c_rates, currents)):
        result = model.predict({"soc": soc, "current": I})
        fig.add_trace(go.Scatter(x=soc, y=result["voltage"], name=f"{c}C ({I:.0f}A)",
                                  line=dict(color=colors[i])), row=1, col=2)

    for i, c in enumerate([0.1, 0.2, 0.5, 1.0]):
        I = c * capacity_ah
        dt = 5.0
        soc_t = 1.0
        ...
        while soc_t > 0.0:
            ...
            soc_t += float(result["dsoc_dt"]) * dt
            total_ah += I * dt / 3600.0
            if total_ah > capacity_ah * 1.1:
                break
        fig.add_trace(go.Scatter(x=ah_delivered, y=voltages, name=f"{c}C discharge",
                                  line=dict(color=colors[i + 1]), showlegend=False), row=2, col=1)

    for i, (c, I) in enumerate(zip(c_rates[1:], currents[1:])):
        result = model.predict({"soc": soc, "current": I})
        fig.add_trace(go.Scatter(x=soc, y=result["power"], name=f"{c}C power",
                                  line=dict(color=colors[i + 1]), showlegend=False), row=2, col=2)
