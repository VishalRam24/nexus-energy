"""EC035 — NaS Battery — F1a SOC-Only — Test Suite"""
...
def test_voltage_drops_with_discharge_current(model):
    currents = np.array([0.0, 20.0, 50.0, 100.0, 200.0])
...
def test_voltage_within_bounds(model):
    currents = np.linspace(-200.0, 200.0, 100)
    for soc in [0.0, 0.5, 1.0]:
        result = model.predict({"soc": soc, "current": currents})
        assert np.all(result["voltage"] >= 1.78)
        assert np.all(result["voltage"] <= 2.08)


def test_ocv_at_full_charge(model):
    """OCV at SOC=1.0 close to V_max ~2.08 V."""
    result = model.predict({"soc": 1.0, "current": 0.0})
    assert 2.00 <= result["ocv"] <= 2.12


def test_ocv_at_empty(model):
    """OCV at SOC=0.0 close to V_min ~1.78 V."""
    result = model.predict({"soc": 0.0, "current": 0.0})
    assert 1.72 <= result["ocv"] <= 1.85
...
def test_soc_derivative_sign(model):
    assert model.predict({"soc": 0.5, "current": 50.0})["dsoc_dt"] < 0
    assert model.predict({"soc": 0.5, "current": -50.0})["dsoc_dt"] > 0


def test_array_inputs(model):
    socs = np.array([0.2, 0.5, 0.8])
    currents = np.array([10.0, 20.0, 30.0])
