"""EC032 -- Zinc-Air Battery -- F1b SOC-Thermal -- Simulation Report"""
...
def generate_report():
    ...
    fig = make_subplots(rows=2, cols=2,
        subplot_titles=["V vs SOC at 1A discharge","R vs Temperature",
                        "Heat Generation vs Current","Effective Capacity vs T"],
        vertical_spacing=0.13, horizontal_spacing=0.10)

    for T, lbl, clr in zip(temps, labels, colors):
        V = model.predict({"soc": soc, "current": I_discharge, "temperature": T})["terminal_voltage"]
    ...
    I_arr = np.linspace(0.1, 5.0, 100)
    Q_arr = [float(model.predict({"soc": 0.5, "current": float(I), "temperature": 298.15})["heat_generation"]) for I in I_arr]
    fig.add_trace(go.Scatter(x=I_arr, y=Q_arr, name="Q vs I (298K)", line=dict(color="#2ca02c")), row=2, col=1)

    C_arr = [float(model.predict({"soc": 0.5, "current": 0, "temperature": T})["effective_capacity"]) for T in T_arr]
    fig.add_trace(go.Scatter(x=T_arr-273.15, y=C_arr, name="Capacity vs T", line=dict(color="#d62728")), row=2, col=2)

    fig.update_layout(title_text="EC032 Zinc-Air Battery F1b SOC-Thermal Simulation Report", height=700)
