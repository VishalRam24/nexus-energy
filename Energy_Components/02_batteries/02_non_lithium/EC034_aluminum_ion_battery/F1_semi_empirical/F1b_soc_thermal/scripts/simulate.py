"""EC034 -- Aluminum-Ion Battery -- F1b SOC-Thermal -- Simulation Report"""
...
def generate_report():
    ...
    temps = [263.15, 278.15, 298.15, 313.15, 333.15]
    labels = ["-10C","5C","25C","40C","60C"]
    ...
    I_arr = np.linspace(0.1, 10.0, 100)
    Q_arr = [float(model.predict({"soc":0.5,"current":float(I),"temperature":298.15})["heat_generation"]) for I in I_arr]
    fig.add_trace(go.Scatter(x=I_arr, y=Q_arr, name="Q vs I (298K)"), row=2, col=1)
    C_arr = [float(model.predict({"soc":0.5,"current":0,"temperature":T})["effective_capacity"]) for T in T_arr]
    fig.add_trace(go.Scatter(x=T_arr-273.15, y=C_arr, name="C(T)"), row=2, col=2)
    fig.update_layout(title_text="EC034 Al-Ion Battery F1b SOC-Thermal Simulation Report", height=700)
