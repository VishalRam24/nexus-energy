"""
EC029 — Nickel-Metal Hydride (NiMH) Battery — F1a SOC-Only Model

Simple semi-empirical voltage model:
    V_terminal = OCV(SOC) - I * R_internal

OCV(SOC) is a 5th-order polynomial fit to a representative NiMH
discharge curve. NiMH cells exhibit a fairly flat plateau around the
nominal 1.2 V/cell potential.

Reference:
    Linden, D. (Ed.). Linden's Handbook of Batteries, 4th ed., 2011,
    Chapter 31 (Nickel-Metal Hydride Batteries).
"""

import numpy as np


class NiMHBatteryF1a:
    """NiMH battery cell model — voltage as a function of SOC and current only."""
