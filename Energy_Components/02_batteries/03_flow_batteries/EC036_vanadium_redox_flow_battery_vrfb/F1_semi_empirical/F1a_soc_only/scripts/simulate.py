"""EC036 — VRFB — F1a — Simulation & HTML Report"""
...
def generate_report():
    ...
    soc = np.linspace(0.1, 0.9, 200)

    # Row 1 Col 1 — Nernst voltage vs SOC
    ...
    # Row 1 Col 2 — stack voltage vs SOC at various discharge rates
    for I in [20, 50, 80, 100]:
        r = model.predict({"soc": soc, "current": float(I)})
        fig.add_trace(
            go.Scatter(x=soc, y=r["stack_voltage"], name=f"I={I} A"),
            row=1, col=2,
        )

    # Row 2 Col 1 — efficiency vs SOC at various currents
    for I in [20, 50, 100]:
        r = model.predict({"soc": soc, "current": float(I)})
        fig.add_trace(
            go.Scatter(x=soc, y=r["efficiency"] * 100.0, name=f"eta I={I}A"),
            row=2, col=1,
        )

    # Row 2 Col 2 — charge/discharge simulation
    # Charge to SOC=0.9, then discharge to SOC=0.1
    dt = 30.0     # seconds
    C_total_C = 40 * 600 * 1.6 * 96485 / 1e3   # rough: not needed for voltage sim
    # Use a simpler capacity: track via Coulombs from rated
    # Rated capacity: I=50A for 2h => 100Ah => C_As = 360000 As
    C_As = 360000.0

    # Discharge from SOC=0.9 to SOC=0.1 at I=50A
    I_dis = 50.0
    soc_sim = [0.9]
    v_stack_sim = []
    t_sim = []
    ...
    while soc_sim[-1] > 0.1:
        ...
        dsoc = -I_dis * dt / C_As
        soc_sim.append(max(0.1, s + dsoc))
        step += 1
        if step > 5000:
            break

    fig.add_trace(
        go.Scatter(x=t_sim, y=v_stack_sim,
                   name=f"Discharge {int(I_dis)}A",
    ...
    fig.update_layout(
        title=f"{info['ec_id']} — {info['name']} — {info['fidelity']} | 40-cell, 600 cm2",
        height=800,
