"""EC039 -- Organic Flow Battery -- F1a SOC-only -- Test Suite"""
...
def test_instantiation():
    ...
    info = m.get_info()
    assert_true(info["ec_id"] == "EC039", "ec_id == EC039")
...
def test_output_keys():
    ...
    m = ComponentModel()
    r = m.predict({"soc": 0.5, "current": 20.0})
...
def test_nernst_midpoint():
    print("Test: Nernst OCV at SOC=0.5 equals E0=1.0V")
    m = ComponentModel()
    r = m.predict({"soc": 0.5, "current": 0.0})
    assert_true(abs(r["V_cell_ocv"] - 1.0) < 1e-5,
                f"V_cell_ocv@SOC=0.5 = 1.0V (got {r['V_cell_ocv']:.6f})")


def test_stack_voltage_proportional():
    print("Test: V_stack_ocv = 20 * V_cell_ocv")
    m = ComponentModel()
    r = m.predict({"soc": 0.5, "current": 0.0})
    assert_true(abs(r["V_stack_ocv"] - 20 * r["V_cell_ocv"]) < 1e-4,
                f"V_stack_ocv = 20*V_cell_ocv")
...
def test_voltage_drops_under_current():
    ...
    r1 = m.predict({"soc": 0.5, "current": 20.0})
...
def test_soc_update():
    ...
    m = ComponentModel()
    r = m.predict({"soc": 0.5, "current": 20.0, "dt": 3600.0})
...
def test_benchmark():
    ...
    for i in range(1000):
        soc = 0.1 + (i % 80) * 0.01
        m.predict({"soc": soc, "current": float(i % 50)})
