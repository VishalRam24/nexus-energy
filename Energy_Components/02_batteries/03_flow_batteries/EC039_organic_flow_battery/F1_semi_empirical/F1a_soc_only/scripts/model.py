"""EC039 -- Organic Flow Battery -- F1a SOC-only (Nernst OCV)"""
import math


class OrganicFlowBatteryModel:
    """
    Organic flow battery SOC-only model (quinone-based redox couples).
    ...
    def __init__(self, params: dict):
        self.E0 = float(params.get("E0", 1.0))
        self.n = int(params.get("n", 1))
        self.T = float(params.get("T", 298.15))
        self.R_int_area = float(params.get("R_int_area", 2.0))
        self.N_cells = int(params.get("N_cells", 20))
        self.A_cell = float(params.get("A_cell", 500.0))
        self.Q_nom_Ah = float(params.get("Q_nom_Ah", 50.0))
