"""
EC039 -- Organic Flow Battery -- F1b SOC-Thermal -- Simulation & HTML Report Generator

OFB operates from 10-40 degC (narrow range due to organic molecule thermal stability).
"""
...
def generate_report():
    ...
    # OFB: 10-40 degC
    temperatures = [283.15, 288.15, 298.15, 308.15, 313.15]
    temp_labels = ["10C (283K)", "15C (288K)", "25C (298K)", "35C (308K)", "40C (313K)"]
    ...
    T_range = np.linspace(283.15, 313.15, 200)
    ...
    currents = np.linspace(0.0, 50.0, 200)
    for i, (T, label) in enumerate(zip(temperatures, temp_labels)):
