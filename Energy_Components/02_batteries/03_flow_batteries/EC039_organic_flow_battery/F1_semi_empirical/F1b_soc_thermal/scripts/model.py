...
Extends F1a (Nernst + ohmic) by adding temperature dependence:
    ...
    P_pump = k_pump * I^2   [W, parasitic pump losses]

Representative chemistry: AQDS/ferricyanide (anthraquinone disulfonate positive,
ferricyanide negative). 2-electron quinone reduction.

    Positive: Fe(CN)6^3- + e- <-> Fe(CN)6^4-      E0 ~ +0.36 V
    Negative: AQDS + 2H+ + 2e- <-> H2AQDS          E0 ~ -0.33 V
    Cell: E0 ~ 0.70 V

Temperature range: 10-40 degC. Upper limit set by organic molecule degradation.
Lower limit set by electrolyte viscosity and kinetics.

References:
    Huskinson, B. et al. (2014). A metal-free organic-inorganic aqueous flow battery.
        Nature 505, 195-198.
    Lin, K. et al. (2015). Alkaline quinone flow battery. Science 349, 1529-1532.
    Kwabi, D. G. et al. (2020). Electrolyte lifetime in aqueous organic flow batteries.
        Chem. Rev. 120, 6467-6489.
"""
...
class OrganicFlowF1b:
    """Organic redox flow battery stack -- voltage as f(SOC, current, temperature)."""
    ...
    def pump_loss(self, current):
        """Parasitic pump power [W]. P_pump = k_pump * I^2."""
        current = np.asarray(current, dtype=float)
    ...
    def heat_generation(self, soc, current, temperature):
        ...
        Includes Joule heating and reversible entropic term.
        """
        current = np.asarray(current, dtype=float)
        temperature = np.asarray(temperature, dtype=float)
