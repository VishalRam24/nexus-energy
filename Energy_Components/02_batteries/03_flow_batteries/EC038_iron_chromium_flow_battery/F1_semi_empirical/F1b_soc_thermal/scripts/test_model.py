"""
EC038 -- Iron-Chromium Flow Battery -- F1b SOC-Thermal -- Test Suite

Physics sanity checks for temperature-dependent ICFB Nernst model.
...
def test_predict_returns_all_keys(model):
    result = model.predict({"soc": 0.5, "current": 40.0, "temperature": 298.15})
...
def test_resistance_increases_at_lower_temperature(model):
    ...
    r_hot = model.predict({"soc": 0.5, "current": 0, "temperature": 338.15})["internal_resistance_cell"]
    assert float(r_cold) > float(r_ref), "R at 15C must be > R at 25C"
    assert float(r_ref) > float(r_hot), "R at 25C must be > R at 65C"


def test_resistance_at_reference(model):
    # R_cell_ref = 4.0 Ohm.cm2 / 500 cm2 = 0.008 Ohm
    R_cell_ref = 4.0 / 500.0
    r = model.predict({"soc": 0.5, "current": 0, "temperature": 298.15})["internal_resistance_cell"]
    assert abs(float(r) - R_cell_ref) < 1e-8


def test_resistance_stronger_temperature_dependence(model):
    """ICFB E_a=20 kJ/mol -> ESR(15C)/ESR(65C) should be >3 (significant Cr kinetics sensitivity)."""
    r_cold = float(model.predict({"soc": 0.5, "current": 0, "temperature": 288.15})["internal_resistance_cell"])
    r_hot = float(model.predict({"soc": 0.5, "current": 0, "temperature": 338.15})["internal_resistance_cell"])
    ratio = r_cold / r_hot
    # RATIONALE: ICFB Cr2+/Cr3+ couple has sluggish kinetics, E_a~20 kJ/mol
    # (Zeng et al. 2015, J. Power Sources 278, 294). At 15C vs 65C:
    # ratio = exp(20000/8.314*(1/288.15-1/338.15)) = exp(20000/8.314*0.000515) ~ exp(1.238) ~ 3.45
    # Threshold >2 confirms meaningful thermal sensitivity from Cr kinetics.
    assert ratio > 2.0, f"R(15C)/R(65C)={ratio:.2f} must be >2 (ICFB Cr kinetics, E_a=20 kJ/mol)"
...
def test_nernst_at_ref_midpoint(model):
    """At SOC=0.5, Nernst = E0(T_ref) = 1.18 V (log term = 0)."""
    e = model.predict({"soc": 0.5, "current": 0, "temperature": 298.15})["e_nernst"]
    assert abs(float(e) - 1.18) < 1e-6, f"Nernst at SOC=0.5, T_ref should be E0=1.18V, got {float(e):.6f}"


def test_nernst_decreases_with_temperature(model):
    """E0(T) = E0_ref + dOCV_dT*(T-T_ref), dOCV_dT < 0 -> higher T gives lower E0."""
    e_cold = model.predict({"soc": 0.5, "current": 0, "temperature": 288.15})["e_nernst"]
    e_hot = model.predict({"soc": 0.5, "current": 0, "temperature": 338.15})["e_nernst"]
...
def test_stack_voltage_lower_at_cold_high_current(model):
    v_cold = model.predict({"soc": 0.5, "current": 80.0, "temperature": 288.15})["stack_voltage"]
    v_hot = model.predict({"soc": 0.5, "current": 80.0, "temperature": 338.15})["stack_voltage"]
    # RATIONALE: At high current, Arrhenius R dominates (R_cold >> R_hot),
    # which outweighs the small E0 decrease from dOCV_dT at higher T.
    # Net: V_stack(cold) < V_stack(hot) at I=80A.
    assert float(v_cold) < float(v_hot), "Stack voltage at 15C must be lower than at 65C (high current)"
...
def test_pump_loss_increases_with_current(model):
    p1 = model.predict({"soc": 0.5, "current": 10.0, "temperature": 298.15})["pump_loss"]
    p5 = model.predict({"soc": 0.5, "current": 50.0, "temperature": 298.15})["pump_loss"]
    assert float(p5) > float(p1), "Pump loss must increase with current"


def test_pump_loss_quadratic(model):
    """P_pump = k_pump * I^2 -> P(2I)/P(I) = 4."""
    p1 = float(model.predict({"soc": 0.5, "current": 20.0, "temperature": 298.15})["pump_loss"])
    p2 = float(model.predict({"soc": 0.5, "current": 40.0, "temperature": 298.15})["pump_loss"])
...
def test_heat_generation_positive_discharge(model):
    q = model.predict({"soc": 0.5, "current": 40.0, "temperature": 298.15})["heat_generation"]
...
def test_efficiency_between_zero_and_one(model):
    eta = model.predict({"soc": 0.5, "current": 40.0, "temperature": 298.15})["efficiency"]
...
def test_efficiency_increases_with_temperature(model):
    ...
    eta_cold = model.predict({"soc": 0.5, "current": 40.0, "temperature": 288.15})["efficiency"]
    eta_hot = model.predict({"soc": 0.5, "current": 40.0, "temperature": 338.15})["efficiency"]
...
def test_zero_current_zero_heat(model):
    ...
    assert abs(float(q)) < 1e-10, "Zero current must give zero heat (no ohmic or entropic term)"
...
def test_soc_extreme_clamped_no_error(model):
    for soc in [0.05, 0.95]:
        result = model.predict({"soc": soc, "current": 40.0, "temperature": 298.15})
        assert np.isfinite(float(result["stack_voltage"]))


def test_invalid_soc_raises(model):
    with pytest.raises(ValueError):
        model.predict({"soc": -0.1, "current": 0, "temperature": 298.15})
...
def test_array_inputs(model):
    ...
    currents = np.array([10.0, 40.0, 80.0])
    temps = np.array([288.15, 298.15, 338.15])
...
def test_benchmark_1000_predictions(model):
    ...
    temps = np.random.uniform(288.15, 338.15, 1000)
