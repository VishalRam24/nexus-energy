"""EC016 — Hydrogen Compressor — F1a — Simulation & HTML Report"""
...
def generate_report():
    ...
    P_out = np.linspace(50, 900, 200)
    P_in_vals = [10.0, 20.0, 30.0]
    colors = ["#1f77b4", "#ff7f0e", "#d62728"]

    for P_in, c in zip(P_in_vals, colors):
        r = model.predict({"P_inlet": P_in, "P_outlet": P_out, "T_inlet": 298.15, "m_dot": 0.014})
        lbl = f"P_in={P_in} bar"
    ...
    fig.update_yaxes(title_text="eta (% of LHV+w)", row=2, col=2)

    fig.update_layout(
        title=f"{info['ec_id']} — {info['name']} — {info['fidelity']} Polytropic Multistage Model<br>"
              f"<sup>4-stage intercooled, eta_p=0.75 | {info['source']}</sup>",
