...
temperatures = [423.0, 443.0, 463.0, 483.0]
...
fig = make_subplots(
    ...
    subplot_titles=[
        ...
        "H3PO4 Conductivity vs Temperature",
    ],
...
# Panel 4: H3PO4 conductivity vs temperature
T_sweep = np.linspace(423, 483, 100)
sigma_vals = [model.predict({"current_density": 0.1, "temperature": float(t)})["acid_conductivity_S_cm"]
              for t in T_sweep]

fig.add_trace(
    go.Scatter(x=T_sweep.tolist(), y=sigma_vals, mode="lines",
               line=dict(color="#9467bd", width=2), name="H3PO4 sigma", showlegend=False),
    row=2, col=2,
)
...
fig.update_yaxes(title_text="H3PO4 Conductivity (S/cm)", row=2, col=2)

fig.update_layout(
    title=dict(
        text=(
            "<b>EC004 -- PAFC -- F1b Polarization-Thermal Model</b><br>"
            "<sup>H3PO4 conductivity (Razaq 1989), Arrhenius cathode kinetics (150-210 C)</sup>"
        ),
        x=0.5,
    ),
    height=800,
