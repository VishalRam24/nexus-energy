...
temperatures = [333.0, 343.0, 353.0, 363.0]
...
# Panel 4: KOH conductivity vs temperature
T_sweep = np.linspace(333, 363, 100)
sigma_vals = [model.predict({"current_density": 0.1, "temperature": float(t)})["koh_conductivity_S_cm"]
              for t in T_sweep]

fig.add_trace(
    go.Scatter(x=T_sweep.tolist(), y=sigma_vals, mode="lines",
               line=dict(color="#9467bd", width=2), name="KOH sigma", showlegend=False),
    row=2, col=2,
)
...
fig.update_layout(
    title=dict(
        text=(
            "<b>EC003 -- Alkaline Fuel Cell -- F1b Polarization-Thermal Model</b><br>"
            "<sup>KOH conductivity (Gilliam 2007), Arrhenius kinetics, temperature-dependent polarization</sup>"
        ),
        x=0.5,
    ),
    height=800,
